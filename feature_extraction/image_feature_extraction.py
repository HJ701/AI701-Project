...
class IntraoralFeatureExtractor(tf.keras.Model):
    def __init__(self):
        ...
        self.base_model = EfficientNetV2B3(include_top=False, weights='imagenet', pooling=None)
    ...
    def call(self, x, training=False):
        features = self.base_model(x, training=training)  # Output shape: [batch_size, H, W, 1536]
        
        # Channel Attention
        channel_attn = self.channel_attention(features)  # Shape: [batch_size, H, W, 1536]
        features = layers.multiply([features, channel_attn])  # Apply channel attention
        
        # Spatial Attention
        spatial_attn = self.spatial_attention(features)  # Shape: [batch_size, H, W, 1536]
        ...
        # Fully Connected Layers for Dimensionality Reduction
        reduced_features = self.fc_reduction(pooled_features, training=training)  # Shape: [batch_size, 1536]
